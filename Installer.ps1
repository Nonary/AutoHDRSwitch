...
if (-not $isAdmin) {
    Start-Process powershell.exe  -Verb RunAs -ArgumentList "-ExecutionPolicy Bypass -NoExit -File `"$($MyInvocation.MyCommand.Path)`" `"$(Join-Path -Path (Get-Location) -ChildPath "HDRToggle.ps1")`" $($MyInvocation.MyCommand.UnboundArguments) $install"
    exit
}
...
function Add-HDRToggleCommand {
    ...
    # Remove any existing commands that contain HDRToggle from the global_prep_cmd value
    $globalPrepCmdArray = Remove-HDRToggleCommand -ConfigPath $confPath
...
$commands = @()
if ($install -eq "True") {
    $commands = Add-HDRToggleCommand
}
else {
    $commands = Remove-HDRToggleCommand 
}
