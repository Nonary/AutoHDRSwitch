...
# to add a command that runs HDRToggle.ps1
...
if (-not $isAdmin) {
    Start-Process powershell.exe  -Verb RunAs -ArgumentList "-ExecutionPolicy Bypass -NoExit -File `"$($MyInvocation.MyCommand.Path)`" `"$(Join-Path -Path (Get-Location) -ChildPath "HDRToggle.ps1")`" $($MyInvocation.MyCommand.UnboundArguments)"
    exit
}
...
# Remove any existing commands that contain HDRToggle from the global_prep_cmd value
function Remove-HDRToggleCommand {
    param (
        # The path to the configuration file
        [Parameter(Mandatory)]
        [string]$ConfigPath
    ...
    # Remove any HDRToggle Commands
    for ($i = 0; $i -lt $globalPrepCmdArray.Count; $i++) {
        if (-not ($globalPrepCmdArray[$i].do -like "*HDRToggle*")) {
...
# Add a new command to run HDRToggle.ps1 to the global_prep_cmd value
function Add-HDRToggleCommand {
    param (
        # The path to the configuration file
        [Parameter(Mandatory)]
        [string]$ConfigPath,

        # The path to the HDRToggle script
        [Parameter(Mandatory)]
        [string]$ScriptPath
    )

    # Remove any existing commands that contain HDRToggle from the global_prep_cmd value
    $globalPrepCmdArray = Remove-HDRToggleCommand -ConfigPath $ConfigPath

    # Create a new object with the command to run HDRToggle.ps1
    $HDRToggleCommand = [PSCustomObject]@{
        do       = "powershell.exe -executionpolicy bypass -file `"$($ScriptPath)`""
        elevated = "false"
        undo     = "powershell.exe -executionpolicy bypass -file `"$($scriptRoot)\HDRToggle-Functions.ps1`" $true"
    }

    # Add the new object to the global_prep_cmd array
    [object[]]$globalPrepCmdArray += $HDRToggleCommand
...
# Invoke the function to add the HDRToggle command
Add-HDRToggleCommand -ConfigPath $confPath -ScriptPath $scriptPath
